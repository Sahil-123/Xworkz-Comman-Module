...
async function loadNotification () {
    ...
    await ajax.get("employee/notification", (response) => {
        ...
        response.toDays.forEach(e => {
            let obj = card.createComplaintCard(e.city, time.convertTo12HourTime(e.createdDate), e.description, e.id);
        ...
        response.older.forEach(e => {
            let obj = card.createComplaintCard(e.city, time.formatDateTime(e.createdDate), e.description, e.id);
            notificationBar.appendChild(obj);
        })
    });
...
function showComplaint(element){
    ...
    window.location.href = "employee/viewComplaint?complaintId="+element.id;
}
